...
public class GamePlayer extends JFrame {
	...
	public void boardPanel1_MouseReleased(java.awt.event.MouseEvent mouseEvent) {
		BoardPanel bp = getBoardPanel1();
		if(computerThinking || bp.gameBoard == null) {
			return;
		}
		// Place the user's move
		try {
			bp.gameBoard.placeUserMove(mouseEvent.getPoint());
			// Place the computer's move if the user didn't finish the game
			if(!(bp.gameBoard.isComputerWinner() || bp.gameBoard.isUserWinner() || bp.gameBoard.isDraw())) {
				ComputerThinkingThread ctt = new ComputerThinkingThread(this, bp.gameBoard, maxLevel);
		...
		// Update the board
		getBoardPanel1().paintComponent(getBoardPanel1().getGraphics());
	}
	
	public void computerDone(TwoPlayerGameBoard computerMove) {
		getBoardPanel1().setBoard(computerMove);
		getBoardPanel1().paintComponent(getBoardPanel1().getGraphics());
		computerThinking = false;
	}

	public void ticTacToeMenuItem_ActionPerformed(java.awt.event.ActionEvent actionEvent) {
		getBoardPanel1().setBoard(new TicTacToeBoard());
		getBoardPanel1().repaint();
		getBoardPanel1().paintComponent(getBoardPanel1().getGraphics());
	}
